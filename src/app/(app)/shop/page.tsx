...
export default function ShopPage() {
  ...
  const earnedTotal = transactions.filter(t => t.amount > 0).reduce((s, t) => s + t.amount, 0)
  const spentTotal = transactions.filter(t => t.amount < 0).reduce((s, t) => s + Math.abs(t.amount), 0)
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 md:px-6 py-8 md:py-10 w-full space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-heading text-xl md:text-2xl font-bold tracking-tight">{t('shop.title')}</h1>
          <p className="text-sm text-zinc-500 font-mono mt-1">{t('shop.desc')}</p>
        </div>
        <div className="flex items-center gap-1.5 px-3 py-2 rounded-md border border-yellow-500/20 bg-yellow-500/10">
          ...
          <span className="font-heading font-bold text-yellow-400">{t('shop.starsCount', { count: stars })}</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1 border-b border-border">
        {[
          ...
          { id: 'HISTORY', label: `${t('shop.transactions.title')} (${transactions.length})`, icon: 'ri-exchange-line' },
        ].map((tTab) => (
          <button
            key={tTab.id}
      ...
      {tab === 'SHOP' && (
        <div className="space-y-3">
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  canAfford ? 'bg-zinc-900 border-border' : 'bg-zinc-900/50 border-border opacity-70',
                )}
              >
                <div className={cn('w-12 h-12 rounded-lg flex items-center justify-center shrink-0 text-xl', item.iconBg)}>
                  <i className={item.icon} />
                </div>
                <div className="flex-1 min-w-0 space-y-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="font-heading font-bold text-sm text-foreground">{t(item.nameKey)}</span>
                    {item.badgeKey && (
                  ...
                  <p className="text-xs font-mono text-zinc-400 leading-relaxed">{t(item.descriptionKey)}</p>
                </div>
                <div className="flex flex-col items-end gap-2 shrink-0">
                  ...
                  >
                    {isPurchasing ? <i className="ri-loader-4-line animate-spin" /> : canAfford ? t('shop.buy') : t('shop.needMore')}
                  </button>
                </div>
              </div>
            )
          ...
          <div className="p-6 rounded-md border border-dashed border-zinc-700 text-center space-y-2">
            ...
            <p className="text-xs font-mono text-zinc-500">{t('shop.comingSoon.desc')}</p>
          </div>
        </div>
      )}
      ...
      {tab === 'HISTORY' && (
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: t('shop.history.balance'), value: `${stars}`, color: 'text-yellow-400' },
              { label: t('shop.history.totalEarned'), value: `+${earnedTotal}`, color: 'text-lime-400' },
              { label: t('shop.history.totalSpent'), value: `-${spentTotal}`, color: 'text-red-400' },
            ].map((s) => (
              <div key={s.label} className="bg-zinc-900 border border-border rounded-md p-3 space-y-1">
                <p className="text-[10px] font-mono text-zinc-500">{s.label}</p>
                <p className={cn('font-heading font-bold text-lg', s.color)}>{s.value}<i className="ri-star-fill text-yellow-400 text-sm ml-1" /></p>
              </div>
            ))}
          </div>
          ...
          {transactions.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 space-y-3">
              ...
              <p className="font-mono text-sm text-zinc-500">{t('shop.history.empty')}</p>
            </div>
          ) : (
            <div className="space-y-1.5">
              {transactions.map((tx) => {
                ...
                return (
                  <div key={tx.id} className="flex items-center gap-3 px-4 py-3 rounded-md bg-zinc-900 border border-border">
                    <i className={cn(meta.icon, meta.color, 'text-sm shrink-0')} />
                    ...
                    <span className={cn(
                      ...
                      tx.amount > 0 ? 'text-lime-400' : tx.amount < 0 ? 'text-red-400' : 'text-zinc-600',
                    )}>
                      {tx.amount !== 0 && (tx.amount > 0 ? '+' : '')}{tx.amount}
                      {tx.amount !== 0 && <i className={tx.amount > 0 ? 'ri-star-fill text-yellow-400 text-xs' : 'ri-star-line text-red-400 text-xs'} />}
                    </span>
