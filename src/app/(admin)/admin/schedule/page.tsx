...
export default function SchedulePage() {
  ...
  async function confirmRemove() {
    ...
    if (res.ok) {
      ...
      setMessage({
        text: `✓ ${removeTarget} removed from ${selectedDate}`,
        type: "success",
      });
    }
    setSaving(null);
  ...
  const loadData = useCallback(async () => {
    ...
    let autoDate = format(startOfToday(), "yyyy-MM-dd");
    for (let i = 0; i < 90; i++) {
      const d = format(addDays(new Date(), i), "yyyy-MM-dd");
  ...
  const past = schedule
    .filter((d) => isBefore(d.date, startOfToday()))
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                {selectedDate < format(new Date(), "yyyy-MM-dd") && (
                  <p className="text-xs font-mono text-yellow-400 flex items-center gap-1.5 mt-1">
                    <i className="ri-history-line" /> Past date — manual
                    schedule only
                  </p>
                )}
              </div>
